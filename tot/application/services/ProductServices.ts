...
export class ProductServices extends IProductService {
  ...
  private isProductInInventory(
    ...
    if (
      ...
      return product.availabilityData.inventories.some(
        x =>
          x.inStockQuantity &&
          x.inStockQuantity > 0 &&
          (x.fulfillmentCenterId === inInventory || x.fulfillmentCenterName === inInventory),
      );
    } else return !!product?.availabilityData?.isInStock;
  }
          ...
          const searchResult: DTO.IProductConnectionDTO = {
            ...
            items: data.items?.map<DTO.IProductDTO>(item => {
              return {
                ...
                isInInventory: this.isProductInInventory(item, inInventory),
              } as DTO.IProductDTO;
            }),
          };
    ...
    return this._context.getProductById(id).then(result => {
      if (result.error) {
        ...
        const product: DTO.IProductDTO = {
          ...
          isInInventory: this.isProductInInventory(result.data, inInventory),
        } as DTO.IProductDTO;
          ...
          item => {
            {
              const _prod = {
                ...
                product: {
                  ...
                  isInInventory: this.isProductInInventory(item.product, inInventory),
                } as DTO.IProductDTO,
              };
              return _prod;
            }
